...
# ロガー初期化
logger = logging.getLogger("blender_graphql_mcp.tools.resolver_compatibility")
...
# VRM関連リゾルバ関数のリスト
VRM_RESOLVERS = [
    'resolve_create_vrm_model', 'resolve_apply_vrm_template', 'resolve_generate_vrm_rig',
    'resolve_assign_auto_weights', 'resolve_add_blend_shape', 'resolve_update_blend_shape',
    'resolve_export_vrm', 'resolve_export_fbx_for_unity', 'resolve_validate_vrm_model',
    'resolve_setup_unity_project', 'resolve_export_to_unity_editor', 'resolve_generate_unity_materials'
]
...
# VRM関連リゾルバの互換性マッピング
# resolvers.pyからVRM関数をマッピング
if _alt_resolver:
    for resolver_name in VRM_RESOLVERS:
        if hasattr(_alt_resolver, resolver_name) and not hasattr(_main_resolver, resolver_name):
            logger.info(f"VRMリゾルバ関数をマッピング: {resolver_name}")
            globals()[resolver_name] = getattr(_alt_resolver, resolver_name)
